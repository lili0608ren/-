# -*- coding: utf-8 -*-
"""7月26日データ実験車両指定実装.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1v9HjtKhnYNGfa0haDLI2dSq3fZUk-19r
"""
...
def binval(v):
    val = pulp.value(v)
    return 1 if val and val > 0.5 else 0
...
print("利用可能車両:")
for idx, name in enumerate(車両候補リスト, 1):
    print(f"{idx}: {name}")
selected = input("使う車両名をカンマ区切りで入力してください（例：ラクティス,ハイエース）: ")
use_cars = [s.strip() for s in selected.split(",") if s.strip()]

# base_vehiclesを「指定した車のみ」に限定
base_vehicles = []
